use crate::{
    app_state::AppState, database::home::Home, method::convention::ErrorData,
...
pub async fn get_home_data(
    ...
    .is_ok()
    {
        let query = "SELECT id, wing, no FROM home WHERE society_id = $1";
        let rows = sqlx::query_as::<_, Home>(query)
            .bind(&society_id)
            .fetch_all(&data.pool)
            .await?;
